...
{
    public class GetTallyProcessStepsQueryHandler : IRequestHandler<GetTallyProcessStepsQuery, List<TallyProcessResponse>>
    {
        private readonly IUnitOfWork _unitOfWork;
        private readonly IMapper _mapper;
        private readonly TextileDbContext _TextileDbContext;
        public GetTallyProcessStepsQueryHandler(
    ...
    GetTallyProcessStepsQuery request,
    CancellationToken cancellationToken)
        {
            var repo = _unitOfWork.Repository<TallyProcessLogs, int>();

            var data = (await repo.GetAllAsync(x => x.CompanyId == request.CompanyId,x=>x.FinanceYearId==request.FinancialYearId,x=>x.ProcessType==request.processType,x=>x.ReferenceNo==request.ReferenceNo)).ToList();
